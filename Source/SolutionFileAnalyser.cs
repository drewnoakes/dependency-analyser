...
{
	/// <summary>
	/// Builds a DependancyGraph from a given Visual Studio solution (.sln) file.
	/// </summary>
	public class SolutionFileAnalyser : DependancyAnalyser
	{
		string _solutionFileName;
		string _solutionPath;

		public SolutionFileAnalyser() { /* constructor for tests only */ }
		...
		public SolutionFileAnalyser(FileStream solutionFileStream)
		{
			...
			string content = ReadEntireStream(solutionFileStream);

			ProjectInfo[] projectInfoList = ExtractProjectInfoFromSolution(content);
		...
		void ProcessProject(ProjectInfo projectInfo)
		{
			...
			string[] dependancies = ExtractDependanciesFromProject(projectFileContent);
			foreach (string dependancy in dependancies)
			{
				AddMessage("- {0}", dependancy);
				DependancyGraph.AddDependancy(projectInfo.Name, dependancy);
			}
		}
		...
		/// </summary>
		string ReadEntireStream(Stream stream)
		{
		...
		internal string[] ExtractDependanciesFromProject(string content)
		{
			ArrayList dependancyList = new ArrayList();
			...
			foreach (XmlNode reference in references)
			{
				...
				if (projectAttribute!=null) 
				{
					...
					dependancyList.Add(reference.Attributes["Name"].Value);
				}
			}
